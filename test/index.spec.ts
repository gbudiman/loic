...
import worker, { TRequestResult, TSessionResult } from '../src/index';
...
describe('executeAsParent', () => {
	...
	it('should orchestrate fanout requests and aggregate results', async () => {
		...
		const request = new IncomingRequest('http://example.com?target_url=http://target.com&fanout=2&requests_per_worker=1');
	...
	it('should handle failed child worker responses', async () => {
		...
		const request = new IncomingRequest('http://example.com?target_url=http://target.com&fanout=1&requests_per_worker=1');
		request.headers.set('X-Service-Token', 'test-token');
		const ctx = createExecutionContext();
	...
	it('should limit fanout and requests per worker to maximum of 100', async () => {
		...
		const request = new IncomingRequest('http://example.com?target_url=http://target.com&fanout=150&requests_per_worker=150');
		request.headers.set('X-Service-Token', 'test-token');
		const ctx = createExecutionContext();
...
describe('executeAsChild', () => {
	...
	it('should make requests to target URL and return results', async () => {
		...
		const request = new IncomingRequest('http://example.com?mode=child&target_url=http://target.com&worker_id=0&requests_per_worker=2&sequence_id=test-seq');
		request.headers.set('X-Service-Token', 'test-token');
		const ctx = createExecutionContext();
	...
	it('should handle failed requests', async () => {
		...
		const request = new IncomingRequest('http://example.com?mode=child&target_url=http://target.com&worker_id=1&requests_per_worker=1&sequence_id=test-seq');
		request.headers.set('X-Service-Token', 'test-token');
		const ctx = createExecutionContext();
